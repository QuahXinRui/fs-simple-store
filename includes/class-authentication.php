...
class Authentication
{
    ...
    public function signup( $email = '', $password = '', $confirm_password = '' ) 
    {
        ...
        //check to make sure password & comfirm_pssword is the same 

    if (
        !empty($password) && //pw is not empty
        !empty($confirm_password) && //confirm pw is not empty
        $password !== $confirm_password
    ){
        $error = "The password and confirm password fields should match";
    }

    //if no error then prced with signup process
    if(!empty($error))
        return $error;
